...
from new.utils.db import insert_query, update_query, select_one_query, update_history_tracker, \
    select_count_query_with_extra_param
        ...
        if response.status_code == 200 and response.headers['Content-Type'] == 'application/pdf':
            if response.url == "http://gstcouncil.gov.in/":
                ...
                insert_query("INSERT INTO alerts (court_name, case_id, page_no, error_message) VALUES (%s, %s, %s, %s)",
                             (court_name, case_id, page_no, 'Failed to download PDF File, its redirected.'))
...
def parser(court_name, page_no, response):
    try:
        ...
        update_query("UPDATE tracker SET total_cases=%s, inserted_cases=0, no_pdf=0, no_text=0, transferred_pdf=0,"
                     "transferred_text=0 WHERE court_name=%s", (len(table_rows), court_name))
        ...
        for row in table_rows:
            ...
            for td in total_td:
                ...
                if str(td['class'][1]) == "views-field-counter":
                    ...
                    if order_and_date is not None:
                        case_id = escape_string((order_and_date[0]).strip())
                        order_date = escape_string((order_and_date[1]).strip())
            ...
            if select_count_query_with_extra_param(str(court_name), str(case_id), 'date', order_date,
                                                   'name_of_applicant', str(applicant), 'state_ut', str(state)):
                pdf_filename = slugify('advance-authority-' + str(case_id) + str(order_date)) + '.pdf'
                text_filename = slugify('advance-authority-' + str(case_id) + str(order_date)) + '.txt'
                pdf_filepath = request_pdf(pdf_url, pdf_filename, court_name, case_id, page_no,
                                           str(applicant_name_for_pdf).strip())
                ...
                if insert_query("INSERT INTO gst_advance_authority (s_no, state_ut, name_of_applicant, questions, "
                                ...
                                "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (s_no, state, applicant, question, case_id,
                                                                           order_date, category_sgst, pdf_url,
                ...
                if update_query("UPDATE gst_advance_authority SET text_data=%s WHERE case_id=%s",
...
def request_data(base_url, court_name):
    ...
    try:
        ...
        # getting last page_ no depends on css so if court site change this can be a problem
        last_page_li = soup.find_all("li", class_="pager-last last")
