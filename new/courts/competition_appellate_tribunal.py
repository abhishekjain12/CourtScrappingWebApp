...
def parser(court_name, bench_id, response):
    ...
    for tr in tr_list:
        ...
        for td in td_list:
            i += 1
            if i == 1:
                case_id = escape_string(str(td.decode_contents()).strip())
            elif i == 2:
                description = escape_string(str(td.text).strip())
            elif i == 3:
                type_ = escape_string(str(td.decode_contents()).strip())
            elif i == 4:
                ...
                date_of_order = escape_string(str(td.text).strip())
        ...
        if select_count_query(str(court_name), str(case_id), 'judgment_date', judgment_date):
            ...
            if insert_query(
                    "INSERT INTO competition_appellate_tribunal (case_id, judgment_date, description, type, "
                    "date_of_order, pdf_url, pdf_filename, text_filename, bench) VALUES (%s, %s, %s, %s, %s, %s, %s, "
                    "%s, %s)", (case_id, judgment_date, description, type_, date_of_order, pdf_url, pdf_filename,
                                text_filename, bench_id)):
            ...
            if update_query("UPDATE competition_appellate_tribunal SET text_data=%s WHERE case_id=%s",
    ...
    try:
        ...
        while datetime.datetime.strptime(str(start_date), "%m/%d/%Y") <= today:
            ...
            while no_tries < NO_TRIES:
                ...
                querystring = {
                    "field_date_of_order_value[min][date]": str(start_date),
                    "field_date_of_order_value[max][date]": str(start_date),
                    "entity_type": "field_content_description_title",
                    "search_key_word": "",
                    "op": "Search"
                }

                response = requests.request("GET", base_url + bench_id, params=querystring, headers=headers,
                                            proxies=proxy_dict).text

                if "no result found" in response.lower():
...
def main(court_name, bench):
    logs.initialize_logger("competition_appellate_tribunal")
    base_url = "https://www.cci.gov.in/orders-commission/"

    headers = {
        'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
        'Cache-Control': "no-cache",
    }
    return request_data(base_url, headers, court_name, str(bench))
