...
def select_count_query_with_extra_param(table_name, case_id, date_col, date_val, extra_parameter_name_1, extra_val_1,
    ...
    try:
        cursor = db.cursor()
        cursor.execute("SELECT count(case_id) FROM " + str(table_name) + " WHERE case_id =%s AND " +
                       str(date_col) + "=%s AND " + str(extra_parameter_name_1) + "=%s AND " +
                       str(extra_parameter_name_2) + "=%s", (str(case_id), str(date_val), str(extra_val_1),
                                                             str(extra_val_2)))
        result = cursor.fetchall()
        cursor.close()
        db.close()
...
def update_history_tracker(court_name, bench=None):
